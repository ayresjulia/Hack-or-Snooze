...
class User {
   ...
   */
	...
	static async login(username, password) {
		...
		existingUser.favorites = response.data.user.favorites.map(s => new Story(story));
		existingUser.ownStories = response.data.user.stories.map(s => new Story(story));
...
class Story {
	...
	async update(user, storyData) {
		const response = await axios.patch(`${BASE_URL}/users/${this.storyId}`, {
			token: user.loginToken,
			story: storyData
		});
